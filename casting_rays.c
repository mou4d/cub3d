...
/*   Updated: 2023/12/01 21:33:08 by wzakkabi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double find_y_horztouch_helper(t_map *map, double x_wallhi, double y_wallhit)
{
...
double  find_y_horztouch(t_map *map, double rayangle, t_angle_facing *a_f)
{
	...
	if (a_f->r_anglefacingright && xstep < 0)
		xstep *= -1;
	if ((a_f->r_anglefacingleft == true && xstep > 0)
		|| (a_f->r_anglefacingright == true && xstep < 0))
		xstep *= -1;
	...
	while (x >= 0 && y >= 0 && y < map->height_map
		&& x <= map->width_map[y] && y_wallhit >= 0)
	{
		if (map->map_s[y][x] == '1')
			break ;
		else
