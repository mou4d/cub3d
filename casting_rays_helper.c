...
/*   Updated: 2023/12/02 07:02:13 by wzakkabi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double find_y_horztouch_helper(t_map *map, t_varb_horz *p,  t_angle_facing *a_f)
{
	double	rtn_value;
	...
	rtn_value =  calculate_distance(map->plr->x, map->plr->y, p->x_wallhit, p->y_wallhit);
	free(p);
	return (rtn_value);
}
...
double  find_y_horztouch(t_map *map, double rayangle, t_angle_facing *a_f)
{
	...
	p->yintercept = floor(map->plr->y / map->size_wall_y_x) * map->size_wall_y_x;
	if (a_f->r_anglefacingdown == true)
		p->yintercept += map->size_wall_y_x;
	p->xintercept = map->plr->x + (((p->yintercept - map->plr->y)) / tan(rayangle));
...
double	find_x_vertical_helper(t_map *map, t_varb_horz *p,t_angle_facing *a_f)
{
	...
	p->x = p->x_wallhit / map->size_wall_y_x;
	p->y = p->y_wallhit / map->size_wall_y_x;
	while (p->x >= 0 && p->y >= 0 && p->y < map->height_map &&  p->x <= map->width_map[p->y] && p->y_wallhit >= 0)
	{
	    if (map->map_s[p->y][p->x] == '1') {
	        break;
	    }
...
double find_x_verticletouch(t_map *map, double rayangle, t_angle_facing *a_f)
{
	...
	p = malloc(sizeof(t_varb_horz));
	p->xintercept = floor(map->plr->x / map->size_wall_y_x) * map->size_wall_y_x;
	if (a_f->r_anglefacingright == true)
	    p->xintercept += map->size_wall_y_x;
	p->yintercept = map->plr->y + ((p->xintercept - map->plr->x)  * tan(rayangle));
